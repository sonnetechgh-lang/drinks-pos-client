...
export default function ProductsPage() {
  ...
  const [formData, setFormData] = useState({ name: '', price: '', categoryId: '', stock: 0, baseUnit: 'UNIT', packageOptions: defaultPackageOptions })
  ...
  const handleOpenModal = (product = null) => {
    if (product) {
      ...
      setFormData({ name: '', price: '', categoryId: categories[0]?.id || '', stock: 0, baseUnit: 'UNIT', packageOptions: defaultPackageOptions })
    }
    setIsModalOpen(true)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...formData,
        packageOptions: formData.packageOptions.map((option, index) => ({
          ...
          <table className="min-w-full text-left">
            <thead className="bg-brand-blue-light/40 text-xs uppercase tracking-[0.18em] text-text-secondary">
              <tr>
                ...
                <th className="px-6 py-4">Brand / Variant</th>
